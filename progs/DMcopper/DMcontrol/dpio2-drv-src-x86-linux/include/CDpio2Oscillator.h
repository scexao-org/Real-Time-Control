...
/* DAVID - the following USED TO BE THIS:
    void CDpio2OscillatorCY22150::clockUpDown();
In that all the ones below had CDPIO2OscillatorCY22150 stuck on the front
*/
    ...
    void acknowlegdePulse();

    void writeByte(UINT8 byte);

    void writeDeviceId(UINT8 byte);

    void setClockRegister(UINT8 deviceAdrs,UINT8 registerAdrs,UINT8 data);

    STATUS setClockGeneratorRegisters(UINT32  div,UINT32  Qt,UINT32  Pb,UINT32  Po,int     doNotUseVco);

    void findBestParameters(UINT32   requestedFrequency, UINT32&  resultingFrequency, UINT32&  bestDiv, UINT32&  bestQt, UINT32&  bestPb, UINT32&  bestPo, int&       doNotUseVco);
